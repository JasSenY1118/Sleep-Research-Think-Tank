...
- `weekly_decision_repository.sh`: local WeeklyLog and Decision CRUD repository using the existing SQLite CLI.
- `weekly_decision_repository_verify.sh`: verifies WeeklyLog and Decision CRUD with placeholder-only temporary data.
...
## WeeklyLog And Decision CRUD Repository

Phase 18C adds a local WeeklyLog and Decision CRUD repository script. It is not connected to the Weekly Progress UI and does not expose API routes.

Available WeeklyLog commands:

```bash
bash db/weekly_decision_repository.sh createWeeklyLog ...
bash db/weekly_decision_repository.sh listWeeklyLogs <db>
bash db/weekly_decision_repository.sh getWeeklyLogById <db> <id>
bash db/weekly_decision_repository.sh updateWeeklyLog <db> <id> <field> <value>
bash db/weekly_decision_repository.sh deleteWeeklyLog <db> <id>
```

Available Decision commands:

```bash
bash db/weekly_decision_repository.sh createDecision ...
bash db/weekly_decision_repository.sh listDecisions <db>
bash db/weekly_decision_repository.sh getDecisionById <db> <id>
bash db/weekly_decision_repository.sh updateDecision <db> <id> <field> <value>
bash db/weekly_decision_repository.sh deleteDecision <db> <id>
```

WeeklyLog `week_start` is required. Decision `date`, `decision`, and `reason` are required. Array-like WeeklyLog fields are stored as JSON text, following the existing local schema pattern.

Run local verification with:

```bash
bash db/weekly_decision_repository_verify.sh
```

The verification script uses a temporary SQLite database and deletes it when finished. It uses placeholder-only records and does not add fake progress, fake decisions, external APIs, calendar/email automation, AI summarisation, semantic search, or website UI database connection.
